...
//     useEffect(() => {
//         const availableSubjects = COMMON_SUBJECTS[className] || [];
//         if (availableSubjects.length > 0 && className !== "Other") {
...
//         const finalClassName = className === "Other" ? customClass : className;
//         const finalSubject = subject === "Custom Subject" ? customSubject : subject;

//         if (!finalClassName.trim() || !finalSubject.trim()) {
//             toast.error('Please fill in all required fields');
//             return;
//         }

//         setLoading(true);
//         const loadingToast = toast.loading("🤖 Generating subject-based quiz...");
//         setMcqs([]);
//         setAnswered({});
//         setSubmitted(false);
//         setScore(0);
...
//         try {
//             const resp = await axios.post(`${BACKEND_URL}/generate-subject-mcqs`, {
//                 className: finalClassName,
//                 subject: finalSubject,
//                 chapterName: chapterName.trim(),
//                 count: questionCount,
//                 customClass: className === "Other" ? customClass : ""
//             }, { timeout: 90000 });
...
//         // save history
//         try {
//             await axios.post(`${BACKEND_URL}/subject-save-history`, {
//                 className: className === "Other" ? customClass : className,
//                 subject: subject === "Custom Subject" ? customSubject : subject,
//                 chapterName: chapterName.trim(),
//                 score: finalScore,
//                 total: mcqs.length
//             });
...
//         try {
//             const resp = await axios.get(`${BACKEND_URL}/subject-quiz-history`);
...
//                                 📚
//                             </div>
//                             <h1 className="text-4xl md:text-5xl font-bold dark:text-white text-gray-900">Subject Based Quiz</h1>
//                         </div>
//                         <p className="text-lg dark:text-gray-100 text-gray-600 max-w-3xl mx-auto">
//                             Generate educational quizzes by class, subject and chapter/topic for comprehensive learning.
//                         </p>
//                     </div>
...
//                                             placeholder="e.g., Kindergarten, Graduate Level, etc."
...
//                                                     </div>
//                                                     <div>
//                                                         <h3 className="font-bold dark:text-white text-gray-900 text-lg">{entry.className} - {entry.subject}</h3>
//                                                         <p className="text-sm dark:text-gray-100 text-gray-500 font-medium">{entry.chapterName}</p>
...
//                                                 {className === "Other" ? customClass : className} - {subject === "Custom Subject" ? customSubject : subject}
//                                             </h2>
//                                             <p className="text-lg font-medium text-gray-700 dark:text-gray-300">{chapterName}</p>
...
//                                         You scored {Math.round((score / mcqs.length) * 100)}% on {chapterName}
//                                     </p>
//                                     <p className="text-lg text-gray-500 dark:text-gray-400 mb-8">
//                                         Subject: {subject === "Custom Subject" ? customSubject : subject} ({className === "Other" ? customClass : className})
//                                     </p>
//                                     <button
...
import axios from "axios";
import toast, { Toaster } from "react-hot-toast";
import { Trophy, Star, Users, ChevronDown, GraduationCap, Book, Lightbulb, EyeOff, RefreshCw, BookOpen } from 'lucide-react';
import Footer from "../Footer";
import Navbar from "../Navbar";
...
export default function SubjectQuiz() {
    ...
    const fetchMCQs = async () => {
        ...
        try {
            ...
            if (resp.data && Array.isArray(resp.data.mcqs) && resp.data.mcqs.length) {
                if (resp.data.fallback) {
                    // API limit hit, show error message instead of fallback questions
                    setApiLimitHit(true);
                    toast.error('API limit reached. Please try again.');
                } else {
                    setMcqs(resp.data.mcqs);
                    toast.success(`✅ ${resp.data.mcqs.length} questions ready!`);
                }
                            ...
                            {submitted && (
                                <div className="bg-transparent dark:text-white overflow-hidden p-12 text-center">
                                    ...
                                    <button
                                        ...
                                        onClick={resetQuiz}
                                    > <BookOpen className="w-6 h-6" />
                                        <span>Start New Quiz</span></button>
                                </div>
                            )}
                        </div>
                    )}
