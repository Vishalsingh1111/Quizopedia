...
export default function ExamQuiz() {
    const [exam, setExam] = useState(POPULAR_EXAMS[0]);
    ...
    const getSelectedExam = () => {
        return exam === "Other" ? customExam : exam;
    };

    const fetchMCQs = async () => {
        const selectedExam = getSelectedExam();
        if (!selectedExam.trim()) {
            showToast('Please enter exam name for "Other" option', 'error');
            return;
        }

        setLoading(true);
        showToast("🤖 Generating exam-based quiz...", 'loading');
        setMcqs([]);
        setAnswered({});
        setSubmitted(false);
        setScore(0);
        ...
        try {
            const response = await fetch(`${BACKEND_URL}/generate-exam-mcqs`, {
                method: 'POST',
                headers: {
            ...
            if (data && Array.isArray(data.mcqs) && data.mcqs.length) {
                setMcqs(data.mcqs);
                if (data.fallback) {
                    showToast('⚠️ Using sample questions (no AI configured)', 'warning');
                } else {
    ...
    const handleSubmit = async () => {
        ...
        // save history
        try {
            await fetch(`${BACKEND_URL}/exam-save-history`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            showToast('Failed to save quiz history', 'error');
        }
    };

    const fetchQuizHistory = async () => {
        try {
            const response = await fetch(`${BACKEND_URL}/exam-quiz-history`);
            const data = await response.json();
            setQuizHistory(data.history || []);
            setShowHistory(true);
        } catch (err) {
            console.error(err);
            showToast('Failed to fetch quiz history', 'error');
        }
    };
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    <div className="text-center mb-12">
                        <div className="flex items-center justify-center mb-4">
                            ...
                            <h1 className="text-4xl md:text-5xl font-bold dark:text-white text-gray-900">Exam-Based Quiz Generator</h1>
                        </div>
                        <p className="text-lg dark:text-gray-100 text-gray-600 max-w-3xl mx-auto">
                            Generate targeted quiz questions for popular Indian competitive exams — choose exam, topic, and question count.
                        </p>
                    </div>
                    ...
                    {mcqs.length === 0 && !showHistory && (
                        <div className="bg-white dark:bg-white/5 dark:backdrop-blur-md dark:text-white rounded-xl shadow-sm border border-gray-100 dark:border-white/10 overflow-hidden p-8 mb-8">
                         ...
                         transition-all bg-white dark:bg-black text-gray-900 dark:text-white"
                                ...
                                {/* Custom Exam Input (shown when "Other" is selected) */}
                                {exam === "Other" && (
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">
                                            Enter Exam Name
                                        ...
                                        <input
                                            ...
                                            placeholder="e.g., UPSSSC PET, RRB JE, etc."
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        <input
                                            ...
                                            placeholder="Enter specific topic or keep 'Miscellaneous' for full syllabus"
                                            className="w-full border border-gray-200 dark:border-white/20 rounded-lg px-4 py-3 
                         transition-all bg-white dark:bg-black text-gray-900 dark:text-white
                         placeholder-gray-500 dark:placeholder-gray-400"
                                            disabled={loading}
                                    ...
                                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                                        Use "Miscellaneous" for a comprehensive mock test covering the entire exam syllabus
                                    </p>
                                </div>
                                        ...
                                        <div key={entry._id || i} className="border border-gray-100 dark:border-white/20 rounded-lg p-6 hover:shadow-sm dark:bg-black">
                                            <div className="flex justify-between items-center">
                                                <div className="flex items-center space-x-4">
                                                    ...
                                                    <div>
                                                        <h3 className="font-bold dark:text-white text-gray-900 text-lg">{entry.exam}</h3>
                                                        <p className="text-sm dark:text-gray-300 text-gray-600">Topic: {entry.topic}</p>
                            ...
                            {submitted && (
                                <div className="bg-transparent dark:text-white overflow-hidden p-12 text-center">
                                    ...
                                    <p className="text-xl text-gray-600 dark:text-gray-400 mb-8">
                                        You scored {Math.round((score / mcqs.length) * 100)}% in {getSelectedExam()}
                                    </p>
                                    <button
